...
func TestResourceNameToId(t *testing.T) {
  ...
  /subnet<subnet_1>  "property"@[] "{"Key":"Name","Value":"mongo"}"^^type:text`))
	...
	tcases := []struct {
		...
		{name: "redis", resourceType: graph.Instance, expectID: "inst_1", ok: true},
		{name: "redis2", resourceType: graph.Instance, expectID: "inst_2", ok: true},
		{name: "mongo", resourceType: graph.Instance, expectID: "inst_3", ok: true},
		{name: "mongo", resourceType: graph.Subnet, expectID: "subnet_1", ok: true},
