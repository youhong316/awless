...
func TestUnmarshalResource(t *testing.T) {
	res := InitResource("inst_1", graph.Instance)
  ...
  /instance<inst_1>  "property"@[] "{"Key":"State","Value":{"Code": 16,"Name":"running"}}"^^type:text`))
	...
	node, err := node.NewNodeFromStrings(graph.Instance.ToRDFString(), "inst_1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadResources(t *testing.T) {
  ...
  /instance<subnet>  "property"@[] "{"Key":"Id","Value":"my subnet"}"^^type:text`))
	...
	expected := []*Resource{
		{kind: graph.Instance, id: "inst_1", properties: Properties{"Id": "inst_1", "Name": "redis"}},
		{kind: graph.Instance, id: "inst_2", properties: Properties{"Id": "inst_2", "Name": "redis2"}},
		{kind: graph.Instance, id: "inst_3", properties: Properties{"Id": "inst_3", "Name": "redis3", "CreationDate": time}},
	}
	res, err := LoadResourcesFromGraph(g, graph.Instance)
	if err != nil {
		t.Fatal(err)
	}
